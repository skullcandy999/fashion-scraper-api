"""
Fashion Scraper API - FULLY OPTIMIZED with Parallel Validation for ALL brands
"""
...
TIMEOUT = 8
MIN_BYTES = 8000
MAX_VALIDATION_WORKERS = 20  # Parallel URL validation
    ...
    try:
        ...
        if r.status_code == 200 and r.content and len(r.content) > min_bytes:
            ...
            if "image" in content_type or len(r.content) > 20000:
                return (url, True, sha1_hash(r.content), metadata)
        return (url, False, None, metadata)
    except Exception:
        return (url, False, None, metadata)
...
def validate_urls_parallel(url_metadata_list, custom_session=None, min_bytes=MIN_BYTES, max_images=5):
    ...
    Returns: list of valid image dicts, preserving order of first valid ones found
    """
    images = []
    seen_hashes = set()
    ...
    # Create a dict to track original order
    ...
    # Build final list
    for url, is_valid, img_hash, metadata in results:
...
def health():
    return jsonify({"status": "ok", "version": "optimized-parallel-v2"})
...
def scrape_maje():
    try:
        ...
        url_list = []
        for i in range(1, 6):
            url_list.append((f"{base_url}images/hi-res/{file_prefix}_F_{i}.jpg?sw=1520&sh=2000", {"type": "model"}))
        url_list.append((f"{base_url}images/packshot/{file_prefix}_F_P.jpg?sw=1520&sh=2000", {"type": "packshot"}))
...
def scrape_mango():
    try:
        ...
        images = [{"url": url, "index": i + 1, "filename": f"{sku}-{i + 1}"} for i, url in enumerate(candidate_urls[:max_images])]
        
        return jsonify({"sku": sku, "formatted_sku": sku, "their_code": their_code, "images": images, "count": len(images), "note": "No validation"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_allsaints():
    try:
        ...
        for pos in range(1, 11):
            for variant in (1, 2):
                url_list.append((make_url(their_code, pos, variant), {"position": pos}))
        
        images = validate_urls_parallel(url_list, custom_session=as_session, min_bytes=20000, max_images=max_images)
...
def scrape_boggi():
    try:
        ...
        for i in range(0, max_images):
            file_part = f"{model_code}.jpeg" if i == 0 else f"{model_code}_{i}.jpeg"
            images.append({"url": BASE_IMG + file_part, "index": i + 1, "filename": f"{sku}-{i + 1}"})

        return jsonify({"sku": sku, "formatted_sku": sku, "model_code": model_code, "color_code": color_code, "images": images, "count": len(images), "note": "No validation"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_kate_spade():
    try:
        ...
        COLOR_MAP = {"500": ["020", "500", "000"], "001": ["001", "000", "020"], "200": ["200", "020", "000"]}
...
def scrape_paul_taylor():
    try:
        ...
        for season in SEASONS:
            for n in NUMS:
                url_list.append((f"{BASE}/{their_base}_{season}_{n}.jpg", {"season": season, "num": n}))
...
def scrape_moose_knuckles():
    try:
        ...
        SUFFIX_ORDER = ["front", "front_category", "front_flat", "back", "side", "left", "right",
                        "detail1", "detail_1", "detail2", "detail_2", "detail3", "onmodel1", "onmodel2", "look1", "look2", "gm"]
...
def scrape_scotch_soda():
    try:
        ...
        rest = sku[2:]
        their = f"{rest.rsplit('-', 1)[0]}_{rest.rsplit('-', 1)[1]}" if "-" in rest else rest
        
        BASE_CDN = "https://scotch-soda.eu/cdn/shop/files"
        SUFFIXES = ["_R_10_FNT_C.png", "_R_10_FNT.png", "_R_10_DTL.png", "_R_10_BCK_C.png",
                    "_FNT.png", "_BCK_C.png", "_BCK.png", "_DTL.png", "_DTL2.png", "-DTL2.png",
                    "_DTL3.png", "-DTL3.png", "_1M.png", "_1M-P.png", "_2M.png", "_3M.png", "_4M.png", "_5M.png", "_6M.png"]
...
def scrape_emporio_armani():
    try:
        ...
        brand, model, fabric, color = code[:2], code[2:8], code[8:13], code[13:]
        ...
        for season in SEASONS:
            for suf in SUFFIXES:
                url_list.append((f"https://assets-cf.armani.com/image/upload/f_auto,q_auto:best,ar_4:5,w_1350,c_fill/{armani_code}_{suf}_{season}.jpg", {"season": season, "suffix": suf}))
...
def scrape_armani_exchange():
    try:
        ...
        for season in SEASONS:
            for suf in SUFFIXES:
                url_list.append((f"https://assets.armani.com/image/upload/f_auto,q_auto:best,ar_4:5,w_1350,c_fill/{ax_code}_{suf}_{season}.jpg", {"season": season, "suffix": suf}))
...
def scrape_michael_kors():
    try:
        ...
        code_part, color = parts[0], parts[1].zfill(4)
        product = code_part[2:] if code_part.startswith(('MCC', 'MC')) else code_part
        ...
        for clr in color_variants:
            for i in range(1, 9):
                url_list.append((f"https://michaelkors.scene7.com/is/image/MichaelKors/{product}-{clr}_{i}?wid=1200&hei=1500&fmt=jpg", {"color": clr, "position": i}))
...
def scrape_patrizia_pepe():
    try:
        ...
        for season in SEASONS:
            for i in range(1, 9):
                url_list.append((f"https://cdn.patriziapepe.com/image/upload/w_1200/q_auto:good/f_auto/CHALCO/SFCC/{product_fabric}/{color}/{season}/{full_code}_{i}.jpg", {"season": season, "position": i}))
...
def scrape_sandro():
    try:
        ...
        url_list = [(f"{BASE}Sandro_{code}_{suffix}.jpg?sw=2000&sh=2000", {"suffix": suffix}) for suffix in SUFFIXES]
...
def scrape_morato():
    try:
        ...
        for their in their_codes:
            ...
            d1, d2 = fn[0], fn[1]
            for i in range(1, 7):
                url_list.append((f"https://cdn.antonymorato.com.filoblu.com/rx/960x,ofmt_webp/media/catalog/product/{d1}/{d2}/{fn}_0{i}.jpg", {"code": their, "position": i}))
...
def scrape_replay():
    try:
        ...
        def parse_replay_sku(s):
            ...
            if match:
                model, fabric, color = match.group(1), match.group(2).replace(' ', '-'), match.group(3)
                return f"{model}_000_{fabric}_{color}", model
            match2 = re.match(r'^([A-Z]+\d+[A-Z]?)([A-Z]\d+[A-Z]?\d*)(\d{3,4})$', s)
            if match2:
                model, fabric, color = match2.group(1), match2.group(2), match2.group(3)
                return f"{model}_000_{fabric}_{color}", model
            return None, None
        ...
        for region in REGIONS:
            for pos in range(1, 11):
                url_list.append((f"{CDN_BASE}/{region}/media/catalog/product/{d1}/{d2}/{cdn_code}_{pos}.jpg{IMG_PARAMS}", {"region": region, "position": pos}))
                ...
                if r.status_code == 200 and len(r.content) >= 8000:
                    ...
                    if "image" in ctype:
                        ...
                        if img_hash in WATERMARK_HASHES:
                            return (url, False, None, metadata)
                        return (url, True, img_hash, metadata)
            except:
                pass
            return (url, False, None, metadata)
        ...
        with ThreadPoolExecutor(max_workers=MAX_VALIDATION_WORKERS) as executor:
            futures = [executor.submit(validate_replay, args) for args in args_list]
            for future in as_completed(futures):
                try:
                    result = future.result()
                    if result[1]:
...
def scrape_joop():
    try:
        ...
        return jsonify({"error": "JOOP scraper module not found"}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_strellson():
    try:
        ...
        return jsonify({"error": "Strellson scraper module not found"}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_woolrich():
    try:
        ...
        return jsonify({"error": "Woolrich scraper module not found"}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_falke():
    try:
        ...
        return jsonify({"error": "Falke scraper module not found"}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_enterprise_japan():
    try:
        ...
        return jsonify({"error": "Enterprise Japan scraper module not found"}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def scrape_generic():
    ...
    return jsonify({"error": f"Unknown brand: {brand}", "available_brands": sorted(list(set(routes.keys())))}), 400
