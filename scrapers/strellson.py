...
def convert_sku(sku):
    """
    ST10014726-10599-402 → 30010599-10014726-402
    - Ukloni ST
...
def search_product(strellson_code):
    """Traži proizvod na Strellson sajtu"""
    # Probaj razne lokalizacije
    bases = [
        "https://strellson.com/de/de",
        "https://strellson.com/int/en",
        "https://strellson.com/at/de",
    ]
    
    for base in bases:
        search_url = f"{base}/search?text={strellson_code}"
        ...
        try:
            ...
            pattern = rf'/p/{re.escape(strellson_code)}'
            match = re.search(pattern, r.text)
            
            if match:
                return f"https://strellson.com{match.group()}"
            
            # Probaj bez boje
            code_no_color = '-'.join(strellson_code.split('-')[:2])
            pattern2 = rf'/p/{re.escape(code_no_color)}-\d+'
            match2 = re.search(pattern2, r.text)
            if match2:
                return f"https://strellson.com{match2.group()}"
        except:
            continue
...
def get_images_from_page(url):
    ...
    try:
        ...
        pattern = r'https://strellson\.com/medias/sys_master/images/images/[^"\'>\s]+'
...
def scrape(sku, max_images=5, validate=True):
    ...
    # Konvertuj SKU
    strellson_code = convert_sku(sku)
    if not strellson_code:
        result["error"] = f"Invalid SKU format: {sku}"
        return result
    
    result["strellson_code"] = strellson_code
    
    # Nađi stranicu proizvoda
    product_url = search_product(strellson_code)
    if not product_url:
        result["error"] = f"Product not found: {strellson_code}"
        return result
